...
type TicketV1 = {
  id: string; // uuid
  ticket_source: string | null;   // ← antes usabas 'folio'
  site_id: string | null;
...
export default function TicketsV1Page() {
  ...
  function buildQuery() {
    ...
    if (term) {
      q = q.or(
        [
          `ticket_source.ilike.%${term}%`,   // ← antes 'folio'
          `site_id.ilike.%${term}%`,
          ...
          >
            ...
            <button
              ...
              style={{
                padding: '8px 16px',
                backgroundColor:
                  page < Math.ceil(total / PAGE_SIZE) ? '#007bff' : '#6c757d',
                color: 'white',
                border: 'none',
                borderRadius: '4px',
                cursor:
                  page < Math.ceil(total / PAGE_SIZE) ? 'pointer' : 'not-allowed',
              }}
            >
              Siguiente →
            </button>
